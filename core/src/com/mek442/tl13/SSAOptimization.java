...
public class SSAOptimization {
	...
	public SSAOptimization(Map<String, Block> pBlockMap) {
		mBlockMap = pBlockMap;
	}

	public OutputHolder optimization() {
		...
		outputHolder.setCfgAfterPhi(printPhiGraph());
		//printBlock();
		...
		outputHolder.setCfgAfterRenaming(printPhiGraph());
		clearBlockVisitations();
		DVNT(mBlockMap.get("B0"));
		clearBlockVisitations();
		traversePhiGraph(mBlockMap.get("B0"));
		clearBlockVisitations();
		outputHolder.setCfgOptimized(printPhiGraph());
		clearBlockVisitations();
		
		replacePhiFunction(mBlockMap.get("B0"));
		
		clearBlockVisitations();
		outputHolder.setCFGRemovePhi(printPhiGraph());
		
		clearBlockVisitations();
		MipsCodeGenerator mips = new MipsCodeGenerator();
		String code = mips.generateCode(mBlockMap, false);
		outputHolder.setOptimiseMIPS(code);
				...
				if (ins.isPhiFunction) {
					...
					for (String string : temp) {
						...
						if (block2.getDominance().size() > 0) {
							...
							if (ins.getParams().containsKey(idom) && !idom.equalsIgnoreCase(block.getLabel())&& isEqualFdom(block2,idom)) {
	...
	private void printBlock() {
		...
		for (String name : mBlockMap.keySet()) {
			...
			for (Instruction ins : block.getInstructions()) {
				if (!ins.isPhiFunction) {
					System.out.println(ins.opCode + " " + ins.getSource1() + " " + ins.getSource2() + " "
							+ ins.getDestication());
				} else {
					System.out.println(ins.getDestication() + " " + ins.opCode + " ( " + getPhiparameter(ins) + " )");
				}
			}
	...
	private void rename(Block pBlock) {
		...
		for (Instruction ins : pBlock.getInstructions()) {
			...
			if (destination != null && ins.isPhiFunction()) {
				...
				/*if (pBlock.getDominanceFrontier().contains(pBlock.getLabel())) {
	...
	private void DVNT(Block pBlock) {
	...
	private void printDominance() {
		for (String name : mBlockMap.keySet()) {
			Block block = mBlockMap.get(name);

			for (String key : block.getDominance()) {
	...
	private void printFrontDominance() {
		for (String name : mBlockMap.keySet()) {
			Block block = mBlockMap.get(name);

			for (String key : block.getDominanceFrontier()) {
	...
	private void printPredecessor() {
		for (String name : mBlockMap.keySet()) {
			...
			for (String key : block.getPredecessor()) {
	...
	private List<String> commonDom(Block pBlock) {
		...
		if (!blocks.isEmpty()) {
			...
			if (pBlock.isLoopHead) {
				if (!blocks.get(1).getDominance().isEmpty() && !blocks.get(1).getDominance().contains(pBlock.getLabel())) {
					for (String dom2 : blocks.get(1).getDominance()) {
				...
				} else if (!blocks.get(0).getDominance().isEmpty() &&!blocks.get(0).getDominance().contains(pBlock.getLabel())) {
	...
	private List<String> getIdomList(Block pBlock) {
		// TODO Auto-generated method stub
		List<String> list = new ArrayList<String>();
		for (String name : mBlockMap.keySet()) {
			Block block = mBlockMap.get(name);
			...
			if (!dominance.isEmpty()) {
				String idom = dominance.get(dominance.size() - 1);
				if (idom.equalsIgnoreCase(pBlock.getLabel())) {
					list.add(block.getLabel());
				}
			}
		}
		return list;
	...
	private String decorateTable(Block pBlock) {
		...
		for (Instruction ins : pBlock.getInstructions()) {
			...
			if(ins.getOpCode().equalsIgnoreCase("cbr")){
				...
				row = ins.getOpCode() + " "+ ins.getSource1() + " - &gt; " + ins.getSource2()+" ,"+ ins.getDestication();
